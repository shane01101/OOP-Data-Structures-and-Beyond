...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		for(Marker qm : quakeMarkers)
		{
			...
			if(eqm.isOnLand())
			{
				if(quakeCount.containsKey(qm.getProperty("country")))
				{
					int count = quakeCount.get(qm.getProperty("country"));
					quakeCount.put((String)qm.getProperty("country"), ++count);
				}
				else
				{
					quakeCount.put((String)qm.getProperty("country"), 1);
				}
			}
			else
			{
				...
				{
					int count = quakeCount.get("OCEAN QUAKES");
					quakeCount.put("OCEAN QUAKES", ++count);
				}
				else
				{
		...
		//System.out.println(Arrays.asList(quakeCount));
